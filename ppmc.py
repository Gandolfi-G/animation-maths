...
class MovingFrameBox(Scene):
    def construct(self):
        text=MathTex(
            "\\text{Déterminer le PPMC de deux nombres}"
        )
...
#  2 Calcul du PGDC        
        ...
        self.wait(2)
        text1=MathTex(
            "\\text{2.On prend les facteurs premiers }", "\\text{ en commun}"
        )   

        pgdc=MathTex(
            "PPMC(24;132) =", " 2"," \cdot"," 2"," \cdot"," 3"," \cdot"," 2"," \cdot"," 11"," = ", "264"
        )
        pgdc.move_to([0,-2,0]) 
        pgdc.set_color_by_tex( " 2",BLUE)     
        pgdc.set_color_by_tex( " 3",GREEN)   
        pgdc.set_color_by_tex( " 11",RED)
        ...
        text2=MathTex(
            "\\text{3.On prend les facteurs premiers }", "\\text{ restants}"
        )
        underline = Underline(mobject=text2[1],buff=0.2)
        self.play(Write(text2), run_time=1)
        self.play (Create (underline),run_time=2)   
        self.wait()
        framebox1 = SurroundingRectangle(nombre11[6],color="PURPLE", buff = .1)
        framebox2 = SurroundingRectangle(nombre21[8],color="PURPLE", buff = .1)
        self.play(            
            Create(framebox1),
            Create(framebox2)
        )    
        ...
        self.play(     
            FadeIn (pgdc[6]),
            FadeIn (pgdc[8]),
            Transform(nombre21[8], pgdc[9]),
            Transform(nombre11[6], pgdc[7]),             
        )         
        self.wait(1) 
